...
from keyboards.inline.high_button import city_residence, scroll_city, choice_answer
...
def survey(message: Message) -> None:
    ...
    :return: None
    """
    bot.set_state(message.from_user.id, UserAnswerState.city_high, message.chat.id)
    bot.send_message(message.from_user.id, 'Введите город в котором собираетесь бронировать номер в отеле')


@bot.message_handler(state=UserAnswerState.city_high)
...
@bot.callback_query_handler(func=lambda high: high.data[:19] in 'city_residence_high')
def host_city(call) -> None:
    ...
    bot.set_state(call.from_user.id, UserAnswerState.arrival_date_high)

    index_city = call.data.split("#")[-1]
    with bot.retrieve_data(call.from_user.id) as data:
        data['city_high'] = scroll_city[int(index_city)]
...
@bot.message_handler(state=UserAnswerState.arrival_date_high)
    ...
    try:
        ...
        if arrival[1] == int(date[1]) and arrival[2] == int(date[2]):
            if int(date[0]) <= arrival[0] <= days_in_month:
                ...
                bot.set_state(message.from_user.id, UserAnswerState.departure_date_high, message.chat.id)

                with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                    data['arrival_date_high'] = message.text
            else:
...
@bot.message_handler(state=UserAnswerState.departure_date_high)
def day_departure_city(message: Message) -> None:
    ...
    try:
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            day_arrival = data['arrival_date_high']
        ...
        if departure[1] == arrival[1] and departure[2] == arrival[2]:
            if arrival[0] < departure[0] <= days_in_month:
                bot.send_message(message.from_user.id,
                                 'Введите максимальную стоимость номера в отеле за сутки в рублях')
                bot.set_state(message.from_user.id, UserAnswerState.high_value_high)

                with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                    data['departure_date_high'] = message.text
            else:
                bot.send_message(message.from_user.id, 'Некорректный ввод даты')
                bot.send_message(message.from_user.id, 'Введите дату отъезда из отеля в формате dd.mm.yyyy')
...
@bot.message_handler(state=UserAnswerState.high_value_high)
def maximum_room_price(message: Message) -> None:
    ...
    if message.text.isdigit():
        if int(message.text) > 1:
            ...
            bot.set_state(message.from_user.id, UserAnswerState.amount_product_high)

            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                data['high_value_high'] = currency_converter
        else:
            bot.send_message(message.from_user.id, 'Максимальная стоимость должна быть больше единицы')
    else:
        bot.send_message(message.from_user.id, 'Стоимость номера может быть только числом')


@bot.message_handler(state=UserAnswerState.amount_product_high)
    ...
    try:
        if message.text.isdigit():
            if int(message.text) < 1:
                ...
                with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
                    data['amount_product_high'] = int(message.text)
                    database.insert_users(user_id=message.from_user.id,
                                          city_residence=data['city_high'],
                                          arrival_date=data['arrival_date_high'],
                                          departure_date=data['departure_date_high'],
                                          maximum_room_price=data['high_value_high'],
                    ...
                    info = f'Выбранные вами настройки:\n' \
                           f'Город проживания - {data["city_high"]}\n' \
                           f'Дата заезда в отель - {data["arrival_date_high"]}\n' \
                           f'Дата отъезда из отеля - {data["departure_date_high"]}\n' \
                           f'Максимальная стоимость номера за сутки - {data["high_value_high"]} $\n' \
                    ...
                    city_stay = data['city_high'].split(', ')
                    id_town = search_id_city(city_stay[0])
                    data['data_hotels'] = data_collection(id_town,
                                                          int(data['arrival_date_high'][:2]),
                                                          int(data['arrival_date_high'][3:5]),
                                                          int(data['arrival_date_high'][6:]),
                                                          int(data['departure_date_high'][:2]),
                                                          int(data['departure_date_high'][3:5]),
                                                          int(data['departure_date_high'][6:]),
                                                          int(message.text), maximum_price=data['high_value_high'])
        ...
        bot.send_message(message.from_user.id, 'По заданной стоимости отели не найдены. '
                                               'Увеличьте стоимость номера в отеле.')
        bot.set_state(message.from_user.id, UserAnswerState.high_value_high)
...
@bot.callback_query_handler(func=lambda exit_from_command: exit_from_command.data in ('yes_high', 'no_high'))
def leaving_the_command(call: Any) -> None:
    ...
    if call.data != 'no_high':
        if data['upper_value_threshold'] < data['amount_product_high']:
